...
$pdf->Cell(0,10,"Ventas del rango $desde a $hasta", 0,1,'C');
$pdf->Ln(5);
...
if ($res->num_rows === 0) {
    ...
    while ($venta = $res->fetch_assoc()) {
        ...
        $pdf->Cell(0,8,"Venta #{$venta['id_venta']} | $tipo | $cliente | {$venta['fecha_venta']} {$venta['hora']} | Total: $" . number_format($venta['total'],2),0,1);
